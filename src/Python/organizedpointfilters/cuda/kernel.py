...
def laplacian_K5_cuda(opc_float, loops=5, _lambda=0.5, **kwargs):
    ...
    for i in range(loops):
        if (i % 2) == 0:
            laplacian_K5(grid_size, block_size, (opc_float_gpu_a, opc_float_gpu_b, opc_height, opc_width, np.float32(_lambda)))
            use_b = True
        else:
            laplacian_K5(grid_size, block_size, (opc_float_gpu_b, opc_float_gpu_a, opc_height, opc_width, np.float32(_lambda)))
            use_b = False
